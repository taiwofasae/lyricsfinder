...
import json
import logging
from embeddings import file_embeddings, s3_embeddings
...
def get_openai_embeddings(phrases, model="text-embedding-3-small"):
    logging.info("Api call to openai for {} items".format(len(phrases)))
...
def fetch_embeddings_for_song(song_id):
    ...
    if results:
        ...
        logging.info("making sure file location is writable before calling api")
        insert_or_update_song_embeddings(song_id, title, [])
...
def fetch_embeddings_for_phrases(phrases):
    ...
    for phrase in phrases:
        insert_or_update_phrase_embeddings(phrase, [])
...
def fetch_embeddings_for_search_phrase(search_id):
    ...
    if results:
        ...
        logging.info("making sure file location is writable before calling api")
        insert_or_update_search_phrase_embeddings(search_id, phrase, [])
